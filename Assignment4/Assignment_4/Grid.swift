...
protocol EngineDelegate{ // task nubmer 3, first part
    func engineDidUpdate(withGrid:GridProtocol)
}

protocol EngineProtocol{ // task number 3 second part
...
class StandardEngine: EngineProtocol { // task number 3, last part.
    var refreshRate: Double
    var refreshTimer: Timer
    var rows: Int
    var cols: Int
    public var grid: Grid {
        willSet{ // called after grid was set
            NotificationCenter.default.post(name: NSNotification.Name("gridUpdated"), object: nil) // to post 'message' that gird was updated ( Whenever the grid is changed...)
        }
    }
    
    public var delegate: EngineDelegate?
    
    internal required init(_ rows: Int, _ cols: Int) { // iniializer of StandardEngine class
        self.rows = rows
        self.cols = cols
        self.refreshRate = 0.0
        self.refreshTimer = Timer()
        NotificationCenter.default.post(name: NSNotification.Name("gridUpdated"), object: nil) // to post 'message' that gird was updated ( Whenever the grid is created ...)
        self.grid = Grid.init(rows, cols, cellInitializer: { (pos:GridPosition) -> CellState in
            return Grid.gliderInitializer(pos: pos) // to init grid with rules from class
        })
        
    }
    static let shared = StandardEngine(10, 10) // create singleton with 10 to 10 by default (taks 3)
    
    func step() -> GridProtocol{
        grid = grid.next() // to call next state of grid
        delegate?.engineDidUpdate(withGrid: grid) // to call engineDidUpdate method in delegate.
        return grid
    }
}
